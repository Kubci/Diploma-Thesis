...
int main(int, char)
{
	...
	cv::Mat image = cv::imread(path + "ctc-PSC\\A-t098.tif", CV_LOAD_IMAGE_ANYDEPTH);
	//cv::Mat image = cv::imread(path + "Angiogenesis\\PhC_TN2DD_MIN1_01.TIF", CV_LOAD_IMAGE_ANYDEPTH);
	//cv::Mat image = cv::imread(path + "QPI\\qpi01.tif", CV_LOAD_IMAGE_ANYDEPTH);
